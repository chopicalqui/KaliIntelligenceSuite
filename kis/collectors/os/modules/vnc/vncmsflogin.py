# -*- coding: utf-8 -*-
"""
run Metasploit auxiliary module vnc_login on each identified in-scope VNC service to perform a password bruteforce
attack
"""
...
logger = logging.getLogger('vncmsflogin')
...
class CollectorClass(BaseVncMsfConsole, ServiceCollector):
    ...
    def get_service_unreachable_regex() -> List[re.Pattern]:
        ...
        """
        return [re.compile("^.*Unable to Connect: The connection was refused by the remote host.*$")]
        ...
        if self.match_nmap_service_name(service):
            ...
            for wordlist in wordlists:
                collectors = self._create_commands(session=session,
                                                   ...
                                                   rport=service.port,
                                                   additional_commands=["set PASS_FILE {}".format(wordlist),
    ...
    def verify_results(self, session: Session,
        ...
        for line in command.stdout_output:
            match_creds = self._re_login.match(line)
            if match_creds:
