# -*- coding: utf-8 -*-
"""
run tool changeme on each identified in-scope HTTP(S) service to test service for default credentials. use optional
argument --user-agent to specify a different user agent string, or optional argument --http-proxy to specify an HTTP
proxy
"""
...
from collectors.os.modules.http.core import BaseHttpChangeme
...
logger = logging.getLogger('httpchangeme')


class CollectorClass(BaseHttpChangeme, ServiceCollector, HostNameServiceCollector):
    ...
    def create_service_commands(self,
        ...
        if self.match_nmap_service_name(service):
            ...
            result = self._create_commands(session=session,
                                           service=service,
                                           collector_name=collector_name,
