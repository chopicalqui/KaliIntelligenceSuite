# -*- coding: utf-8 -*-
"""run tool nmap with all safe NSE scripts on all identified in-scope HTTP(S) services"""
...
logger = logging.getLogger('httpnmap')
...
class CollectorClass(BaseHttpNmap, ServiceCollector, HostNameServiceCollector):
    ...
    def create_service_commands(self,
        ...
        if address and self.match_nmap_service_name(service):
            collectors = self._create_commands(session, service, collector_name, ["http-* and (safe or discovery)"])
        return collectors
