# -*- coding: utf-8 -*-
"""
run tool davtest on each identified in-scope HTTP(S) service that support HTTP method PUT (use collector httpnmap to
identify the supported HTTP methods). if credentials for basic authentication are known to KIS, then they will be
...
logger = logging.getLogger('httpdavtest')
...
class CollectorClass(BaseHttpCollector, ServiceCollector, HostNameServiceCollector):
    ...
    def get_failed_regex() -> List[CommandFailureRule]:
        ...
        """
        return [CommandFailureRule(regex=re.compile("^OPEN\s+FAIL:\s+.*$"),
    ...
    def create_service_commands(self,
        ...
        """
        collectors = []
        command = self._path_davtest
        if self.match_nmap_service_name(service):
            has_put = len([item.name for item in service.service_methods if item.name == "PUT"]) > 0
            if has_put:
                has_move = len([item.name for item in service.service_methods if item.name == "MOVE"]) > 0
