...
scanning top X UDP ports; interesting for scanning interesting UDP ports; all for scanning all UDP ports; or a list of
port numbers/ranges (e.g., 0-1024 8080) to scan just those UDP ports
"""
...
logger = logging.getLogger('udpnmapdomain')
...
class CollectorClass(BaseNmap, DomainCollector):
    ...
    def create_domain_commands(self,
        ...
        if host_name.name is None:
            ...
            ipv4_nmap_options += list(self._nmap_config.nmap_udp_options)
            ipv6_nmap_options += list(self._nmap_config.nmap_udp_options)
            ...
            arguments = self.get_commandline_argument_value("udpnmapdomain")
            results += self._create_domain_commands(session=session,
                                                    ...
                                                    interesting_ports=self._nmap_config.udp_interesting_ports,
                                                    ipv6=False)
            results += self._create_domain_commands(session=session,
                                                    host_name=host_name,
