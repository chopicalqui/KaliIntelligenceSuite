# -*- coding: utf-8 -*-
"""
run tool kismanage on each identified in-scope email address to determine pastes (using haveibeenpwned.com) in which
...
logger = logging.getLogger('haveibeenpaste')
...
class CollectorClass(BaseKisImportEmail, EmailCollector):
    ...
    def api_credentials_available(self) -> bool:
        ...
        """
        return self._api_config.config.get("haveibeenpwned", "api_pasteaccount_url") and \
    ...
    def verify_results(self, session: Session,
        ...
        for json_object in command.json_output:
            if "Source" in json_object:
                pastes = json_object["Source"]
                self.add_additional_info(session=session,
                                         command=command,
                                         name=HaveIBeenPwnedPasteAcccount.NAME,
                                         values=pastes,
                                         email=command.email,
                                         source=source,
                                         report_item=report_item)
