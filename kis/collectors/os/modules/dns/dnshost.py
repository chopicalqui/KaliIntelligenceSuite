# -*- coding: utf-8 -*-
"""
run tool host on each collected in-scope host name (e.g., www.megacorpone.com) and second-level domain (e.g.,
megacorpone.com) to resolve their IPv4/IPv6 addresses using the operating system's DNS server. use optional argument
--dns-server to explicitly specify another DNS server
"""
...
class CollectorClass(BaseDnsHost, DomainCollector):
    ...
    def start_command_execution(self, session: Session, command: Command) -> bool:
        ...
        """
        collector_name = os.path.splitext(os.path.basename(collectors.os.modules.dns.dnshostpublic.__file__))[0]
