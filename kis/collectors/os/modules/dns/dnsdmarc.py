...
obtain DMARK information. use optional argument --dns-server to explicitly specify another DNS server
"""
...
logger = logging.getLogger('dnsdmark')
...
class CollectorClass(BaseDnsCollector, DomainCollector):
    ...
    def create_domain_commands(self,
        ...
        if host_name and host_name.name is None:
            dkim_name = "_dmarc"
            dkim_host_name = session.query(HostName).filter_by(domain_name_id=host_name.domain_name_id,
                                                               name=dkim_name).one_or_none()
            if not dkim_host_name:
                dkim_host_name = HostName(domain_name_id=host_name.domain_name_id,
                                          name=dkim_name)
            ...
            collector = self._get_or_create_command(session, os_command, collector_name, host_name=dkim_host_name)
