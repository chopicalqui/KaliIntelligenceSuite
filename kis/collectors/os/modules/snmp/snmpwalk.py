# -*- coding: utf-8 -*-
"""
run tool snmpwalk on each identified in-scope SNMP service where access credentials are known to KIS (use snmphydra
...
class CollectorClass(BaseSnmpCollector, ServiceCollector):
    ...
    def _get_commands(self,
        ...
        if ipv4_address:
            for version in ['1', '2c']:
                os_command = [command, '-v', version, '-Os']
                if community_string:
                    os_command.extend(['-c', community_string])
                os_command.append("{}:{}".format(ipv4_address, service.port))
