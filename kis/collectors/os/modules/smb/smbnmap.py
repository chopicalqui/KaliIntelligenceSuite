# -*- coding: utf-8 -*-
"""run tool nmap with all safe and discovery NSE scripts on all identified in-scope SMB services. if credentials for
SMB authentication are known to KIS, then they will be automatically used. alternatively, use optional arguments -u,
-p, and -d to provide a user name, a password/NTML hash, and a domain/workgroup for authentication"""
...
class CollectorClass(BaseSmbNmap, ServiceCollector):
    ...
    def create_service_commands(self,
        ...
        if self.match_nmap_service_name(service):
            nse_scripts = ["(smb-* or smb2-*) and (safe or discovery) or smb-enum-users"]
