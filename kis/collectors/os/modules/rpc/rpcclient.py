# -*- coding: utf-8 -*-
"""
run tool rpcclient on each identified in-scope RPCBIND or SMB service. if credentials are known to KIS, then they will
be automatically used. alternatively, use optional arguments -u and -p to provide a user name and password
"""
...
from collectors.os.modules.netbios.core import NbtNsServiceDescriptor
...
class CollectorClass(BaseCollector, ServiceCollector):
    ...
    def __init__(self, **kwargs):
        super().__init__(priority=61400,
                         timeout=0,
                         service_descriptors=[SmbServiceDescriptor(), NbtNsServiceDescriptor()],
                         **kwargs)
    ...
    def _get_commands(self,
        ...
        if self.match_nmap_service_name(service):
            ...
            if user or password:
                ...
                os_command.extend(["-U", '{}%{}'.format(user, password)])
            os_command.append(address)
    ...
    def create_service_commands(self,
        ...
        """
        collectors = []
        command = self._path_rpcclient
        if service.protocol == ProtocolType.tcp and self.match_nmap_service_name(service):
